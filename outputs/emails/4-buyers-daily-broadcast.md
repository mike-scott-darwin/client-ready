---
type: reference
status: active
date: 2026-02-04
purpose: Daily email templates for buyer list
trigger: Day 11+ after purchase (post-welcome sequence)
frequency: Daily
---

# Daily Broadcast Templates

**The compound advantage most coaches ignore: daily emails to your customer list.**

---

## Why Daily

From Miles Stutz:
> "Your customer list is your ATM. You write an email, you make money. But only if you show up daily."

**The math:**
- Weekly emailers: 52 touchpoints/year
- Daily emailers: 365 touchpoints/year
- 7x more opportunities to ascend buyers

**The psychology:**
- Buyers forget you within 72 hours
- Daily = always top of mind when ready to buy
- Story-first means you're not "that pitch person"

---

## The Framework

Every email follows the same structure:

### 1. Story (60% of email)
- Personal experience
- Client story
- Lesson learned
- Something that happened today/this week

**Not:** Generic advice, listicles, "5 tips for..."

### 2. Offer (30% of email)
- One product from your ladder
- Rotate through offers across the week
- Match story to offer when natural

### 3. CTA (10% of email)
- Single, clear action
- Link or reply instruction
- No multiple options

---

## Weekly Offer Rotation

| Day | Offer | Price | Link Text | URL |
|-----|-------|-------|-----------|-----|
| Monday | Front-end | $27 | "Get the System" | https://clientreadyoffer.com/implement |
| Tuesday | Sales Page Kit | $67 | "Get the Sales Page Kit" | https://clientreadyoffer.com/implement |
| Wednesday | DFY Offer Build | $197 | "Get it built for you" | https://clientreadyoffer.com/croto1-dfy |
| Thursday | Community | $47/mo | "Join the Community" | https://www.skool.com/high-ticket-playbook-9467/about |
| Friday | Accelerator | $5K | "Reply BUILD" | Reply trigger (no link) |
| Saturday | The Weekly Playbook | $37/mo | "Get What's Working Now" | https://clientreadyoffer.com/newsletter |
| Sunday | Free value | — | "Reply and tell me" | Reply trigger (no link) |

**Note:** Friday's backend pitch is soft (reply trigger, not direct link). This qualifies serious buyers.

---

## Example Emails (One Per Day)

### Monday — Front-End ($27)

**Subject:** The coach who couldn't explain what she does

---

Had a call yesterday with a coach. Let's call her Lisa.

15 years of HR experience. Helped dozens of people navigate career transitions. Real expertise.

I asked: "What do you do?"

She said: "I help people step into their authentic leadership."

I said: "What does that mean?"

She paused. Then: "I don't know how to explain it."

That's the problem.

Lisa doesn't have a skills problem. She doesn't have an experience problem. She has an articulation problem.

Her offer is trapped in her head. It makes sense to HER but sounds like word salad to everyone else.

This is why coaches struggle to sell. Not because they're bad at coaching. Because they can't explain what they do in one sentence.

If that's you — the Client Ready Offer System fixes it in one afternoon.

5 AI prompts that extract what you actually do and turn it into an offer document you can send to anyone.

$27: https://clientreadyoffer.com/implement

Michael

P.S. — Lisa went through the system last week. Her new one-liner: "I help first-time managers stop losing their best employees in the first 90 days." Night and day.

---

### Tuesday — Sales Page Kit ($67)

**Subject:** I stared at the blank page for 3 hours

---

True story from when I started:

I knew what I wanted to sell. I had the offer figured out. All I needed was a landing page.

I opened Carrd. Stared at the blank page.

3 hours later, I had a headline and two sentences.

Then I deleted them.

The blank page is brutal. Not because you don't know what to say — because you have TOO MANY options. Every word feels like a commitment.

Here's what I wish I knew:

The best marketers don't start from scratch. They start from something that works and make it theirs.

Better still: they don't even write it. They let AI fill it in from the offer they already built.

No blank page. No paralysis. Paste your offer in, get a finished page out.

That's the Plug & Play Sales Page Kit + 1-Hour AI Fill-In System — 3 proven page layouts, a 30-day email sequence, 50+ headlines, and the AI prompts that auto-fill all of it from your offer doc.

You don't invent. You fill in. One hour, not one week.

$67: https://clientreadyoffer.com/implement

Michael

---

### Wednesday — DFY Offer Build ($197)

**Subject:** The difference between "knowing" and "doing"

---

Everyone knows what to do.

Eat less, move more = weight loss.
Spend less than you earn = financial freedom.
Extract your offer, validate, launch = coaching business.

The information is free. It's everywhere. You could figure it out from YouTube alone.

So why don't people do it?

Because knowing isn't doing.

You have the extraction prompts. You know what a good offer looks like. But your ICP doc is half-finished. Your sales doc doesn't exist. Your ad hooks don't exist.

The DFY Offer Build fixes this in 48 hours.

You answer 11 questions (10-15 minutes). I build your complete offer package — ICP, offer doc, ready-to-send sales doc, 5 ad hooks. Reviewed by me personally before it hits your inbox.

$197 one-time. 48-hour delivery.

If you've been "knowing" for a while and want to start "having": https://clientreadyoffer.com/croto1-dfy

Michael

---

### Thursday — Community ($47/mo)

**Subject:** The loneliest part of building

---

Here's what nobody tells you about building a coaching business:

It's lonely.

Your friends don't get it. Your family thinks you should "just get a real job." The online gurus make it look easy while you're struggling.

You don't need more courses. You need people who are in it with you.

That's the Client Ready Community.

Not a course. Not a program. A room full of coaches building their offers, sharing what works, and asking "is this right?"

What you get:
- Weekly hot seat calls with me (live)
- Sprint curriculum (Extract → Validate → Build → Launch)
- DFY templates of the month
- DM access to me
- See what's working for others in real-time

$47/month. Cancel anytime. No contracts.

If building alone is getting old: https://www.skool.com/high-ticket-playbook-9467/about

Michael

---

### Friday — Backend ($5K)

**Subject:** When you're ready for the next level

---

Most of my emails are about the $27-$197 range.

Today's different.

If you've been through the Client Ready system... if you've validated your offer... if you're making sales but want to scale faster...

There's a level beyond templates and courses.

It's called partnership.

My 1:1 Funnel Build is for coaches who:
- Already validated (you have buyers)
- Want speed + expertise (not DIY)
- Are ready to invest $5K+ to get it right

What you get:
- Complete funnel architecture built WITH you
- Offer refinement and positioning
- Full copy direction
- Traffic and scaling strategy
- Ongoing support until you hit your revenue goal

This isn't for everyone. Most people should stay in the $27-$197 range and the community until they've proven the offer.

But if you're past that stage and want a partner, not a product...

Reply "BUILD" and tell me where you're at.

I'll let you know if it makes sense to talk.

Michael

P.S. — No pressure. No sales call if you're not ready. Just a conversation.

---

### Saturday — The Weekly Playbook ($37/mo)

**Subject:** What's working right now

---

The market changes fast.

What converted last month might not convert this month. The hook that worked in January is saturated by March.

Most coaches are building off last year's playbook. They wonder why results dip.

That's why I put together The Weekly Playbook — a monthly deep-dive on what's actually converting right now for Client Ready members.

Each month you get:
- A breakdown of a tested offer, funnel, or strategy
- Templates and swipe files from real campaigns
- What I'm seeing work across different niches

Not theory. Not predictions. What's working today.

$37/month. Cancel anytime. Delivered to your inbox on the 1st.

If you want to stay current instead of guessing: https://clientreadyoffer.com/newsletter

Michael

---

### Sunday — Free Value (No Pitch)

**Subject:** The question that changes everything

---

No pitch today. Just something I've been thinking about.

The most successful coaches I know all asked themselves the same question early on:

"What problem would I solve for free?"

Not "what makes money." Not "what's trending." Not "what does the market want."

What would you solve even if no one paid you?

That's your offer.

Everything else is just packaging.

Have a good Sunday.

Michael

P.S. — If you know what that is for you, reply and tell me. I'm curious.

---

## Story Sources

When you're stuck on what to write:

| Source | Example |
|--------|---------|
| Client conversations | "I was on a call yesterday with..." |
| Skool questions | "Someone asked in the community..." |
| Your own journey | "When I started, I used to..." |
| Mistakes you made | "The biggest mistake I see..." |
| Things you read | "I was reading [book] and..." |
| Observations | "I noticed something interesting..." |
| Wins to celebrate | "Got a message from a client who..." |

**Rule:** If it happened in the last 48 hours, it's worth writing about.

---

## Backend Seeding Strategy

The $5K Accelerator isn't pitched directly. It's seeded through:

1. **Friday emails** — Soft pitch with reply trigger
2. **Transformation stories** — Mention working closely with you
3. **Limitations of DIY** — "If you want to go faster..."
4. **Qualifying language** — "For coaches who've already validated..."

**The goal:** Buyers self-select by replying. No sales page needed.

When someone replies "BUILD":
1. Ask about their current situation
2. Qualify (do they have a validated offer?)
3. If yes, book a call or send Loom walkthrough
4. If no, point them to DFY or community first

---

## Writing Process (15-20 min)

1. **Pick your story** (2 min) — What happened recently that illustrates a point?
2. **Write the story** (8 min) — 150-200 words, conversational
3. **Connect to offer** (3 min) — Natural transition, not forced
4. **Add CTA** (1 min) — One link or one reply instruction
5. **Write subject line** (1 min) — Curiosity or specificity

**Send time:** Same time every day (builds reader habit)

---

## Metrics to Track

| Metric | Target |
|--------|--------|
| Open rate | 25-40% |
| Click rate | 2-5% |
| Unsubscribe rate | <0.5% per email |
| Reply rate | 1-2% |
| Revenue per email | Track monthly |

---

## Segmentation

As buyers ascend, remove them from pitches for products they own:

| Bought | Stop Pitching |
|--------|---------------|
| Sales Page Kit ($67) | Tuesday emails skip offer section |
| DFY ($197) | Wednesday emails skip offer section |
| Community ($47/mo) | Thursday emails skip offer section |
| The Weekly Playbook ($37/mo) | Saturday emails skip offer section |
| Accelerator ($5K) | Friday emails become value-only |

**Tag-based logic in your email tool.** The story stays; just swap the CTA.

---

## Common Mistakes

1. **Emailing prospects, not buyers** — Daily rhythm is for CUSTOMERS only
2. **Pitching without story** — Every email needs the story first
3. **Multiple CTAs** — One email, one action
4. **Skipping days** — Consistency > perfection
5. **Over-producing** — 200 words > 2000 words
6. **Being generic** — Specificity beats broad advice

---

## Integration with Funnel

```
Buyer Journey:
$27 Purchase → 10-Day Welcome → Daily Broadcast → Backend ($5K)

Where daily emails fit:
├── Day 1-10: Welcome sequence (automated)
├── Day 11+: Daily broadcast (manual or batched)
└── Ongoing: Story + rotating offer + backend seeds
```

---

## See Also

- [welcome-sequence-10-day.md](./2-buyers-welcome-10-day.md) — Pre-broadcast sequence
- [install-offer-recovery-sequence.md](../../reference/domain/classroom/downloads/install-offer-recovery-sequence.md) — Bump recovery
- [oto-recovery-sequence.md](../../reference/domain/classroom/downloads/oto-recovery-sequence.md) — OTO recovery
- [community-recovery-sequence.md](../../reference/domain/classroom/downloads/community-recovery-sequence.md) — Community downsell
- [email-rhythm.md](../../reference/domain/funnel/email-rhythm.md) — Strategy overview
