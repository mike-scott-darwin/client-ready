---
type: reference
status: active
date: 2026-02-04
purpose: Buyer welcome sequence (relationship only)
trigger: Purchased $27 Client Ready Offer System
duration: 10 days
note: Relationship-first with soft ascension in iron strike window (Days 5-9). Consumption branch on Day 3.
---

# 10-Day Welcome Sequence

**Trigger:** Purchased $27 Client Ready Offer System
**Goal:** Build relationship, deliver quick wins, establish trust
**Tone:** Miles Stutz directness + Michael Scott warmth
**Emails:** 10 over 10 days

---

## Design Principle

**This sequence is relationship-first with soft ascension in the iron strike window.**

Research across 6+ practitioners shows ascension probability peaks in the first 7-21 days after purchase. This sequence builds relationship AND includes soft ascension touchpoints on Days 5, 7, and 9.

Hard upsell pitches are still handled by parallel recovery sequences:
- Bump Recovery (Days 2, 4, 6) → missed bumps
- OTO Recovery (Days 3, 5, 7) → Sprint/DFY
- Community Recovery (Day 8) → OTO 3 downsell

Day 3 branches based on product consumption — different emails for openers vs non-openers.

See `decisions/2026-02-14-ecosystem-architecture-iron-strike.md` for full rationale.

---

## Sequence Overview

| Day | Subject | Focus | Ascension |
|-----|---------|-------|-----------|
| 1 | You're in — here's your first win | Quick win + access | — |
| 2 | Why I do this (honest answer) | Origin story | — |
| 3a | Now that you've started... | Advanced tips (if opened product) | Consumption branch |
| 3b | Haven't started yet? | Quick start (if NOT opened product) | Consumption branch |
| 4 | The mistake that cost me 6 months | Common mistake | — |
| 5 | The 2-minute test for your offer | Quick tip | Soft close (Sprint) |
| 6 | From stuck to first client in 30 days | Transformation story | — |
| 7 | What my morning actually looks like | Behind the scenes | Soft close (Sprint/Blueprint) |
| 8 | "What if I'm not ready?" | FAQ / objection | — |
| 9 | What happens after $27 | The roadmap | Explicit CTA (Sprint vs Blueprint) |
| 10 | Come hang out | Community invite | — |

---

## Day 1: Welcome + Quick Win

**Subject:** You're in — here's your first win

---

Hey [NAME],

You just grabbed the Client Ready Offer System. Smart move.

Before you dive into the full guide, here's one thing you can do in the next 10 minutes:

**Open the first AI prompt and answer just the "Zone of Genius" question.**

Don't overthink it. Don't try to be clever. Just answer honestly.

That single answer will shape everything else. Most people skip this and wonder why their offer feels generic.

You won't be most people.

Access your materials here: [LINK]

Talk soon,
Michael

P.S. — If you have questions, reply to this email. I read everything.

---

## Day 2: Origin Story

**Subject:** Why I do this (honest answer)

---

Hey [NAME],

People ask why I help coaches build offers.

Here's the truth: I almost burned my business down.

Not financially. Emotionally.

I was doing everything "right" — posting content, running ads, booking calls. But I hated every minute of it. The offer didn't fit. I was growing into pain.

Then I figured out the extraction process. Found the thing I actually wanted to do. Built an offer around THAT.

Everything changed.

That's why the Client Ready system starts with extraction, not tactics. Tactics on a broken foundation just accelerate burnout.

So when you go through the prompts, be honest with yourself. The right offer doesn't feel like pushing. It feels like pulling.

Keep going,
Michael

---

## Day 3: Consumption Branch

**Day 3 sends one of two emails based on whether the buyer has accessed the product.**

### Day 3a: Advanced Tips (if opened product)

**Subject:** Now that you've started — get the most out of Prompt 3

---

Hey [NAME],

I can see you've started going through the system. Nice.

Quick tip that most people miss:

**Prompt 3 (the pain points extraction) is where the money is.**

Most coaches rush through it and write generic answers like "they want more clients" or "they feel stuck."

That's not a pain point. That's a category.

A real pain point sounds like something a real person would say out loud at 11pm:

- "I've been posting for a year and nobody's buying"
- "I can't explain what I do without sounding like every other coach"
- "I know I'm good at this but I can't figure out what to charge"

Specific. Emotional. Real.

If your Prompt 3 answers feel vague, go back and rewrite them in the voice of your actual client. What would they text their best friend?

That's the version that sells.

Reply with your best pain point — I'll tell you if it's specific enough.

Michael

### Day 3b: Quick Start (if NOT opened product)

**Subject:** Haven't started yet? Here's the 5-minute version

---

Hey [NAME],

Hey — noticed you haven't jumped into the system yet.

No judgment. Life gets busy.

But here's the thing: the longer you wait, the less likely you'll start. That's not an opinion — it's just how motivation works.

So here's the **5-minute version**:

**Step 1:** Open the guide. [Here's your access link.]

**Step 2:** Skip to the first AI prompt.

**Step 3:** Answer the Zone of Genius question. Just that one. Don't read ahead. Don't try to be perfect.

That single answer starts the whole extraction process. Everything else builds on it.

5 minutes. One question. That's it for today.

Reply and tell me your answer — I'll give you feedback.

Michael

---

## Day 4: Common Mistake

**Subject:** The mistake that cost me 6 months

---

Hey [NAME],

Biggest mistake I see coaches make:

Starting from scratch.

Blank landing page. Blank email sequence. Blank everything.

It's not just slow — it's demoralizing. You stare at the cursor and wonder if you're even doing this right.

I did this for 6 months before I realized: the best creators don't start from scratch. They start from something that works and make it their own.

Whether it's templates, swipe files, or just a structure to follow — having a starting point changes everything.

That's why the Client Ready system gives you the prompts, not just the concepts. You're not inventing. You're extracting.

Same principle applies to everything downstream: landing pages, emails, ads.

Don't stare at blank pages. Find good examples and adapt them.

Michael

---

## Day 5: Quick Tip

**Subject:** The 2-minute test for your offer

---

Hey [NAME],

Quick tip today.

**The "Mom Test" for your offer:**

Explain your offer to someone who knows nothing about coaching. Your mom. Your neighbor. Anyone outside the industry.

If they can repeat back what you do and who it's for... you're clear.

If they look confused or say "so you're like a life coach?"... you need to sharpen.

Clarity isn't about being clever. It's about being obvious.

Try it today. Let me know what happens.

Michael

P.S. — If you've done the test and your offer passed, you're ahead of most people. The next step is building the system around it. That's what the Client Ready Sprint does — we build it together in 30 days.

---

## Day 6: Transformation Story

**Subject:** From stuck to first client in 30 days

---

Hey [NAME],

James came to me frustrated.

He had expertise — 15 years in corporate finance. He knew he could help entrepreneurs with their money. But he couldn't articulate the offer. Couldn't price it. Couldn't sell it.

His first attempt: "I help business owners with their finances."

Vague. Boring. Unsellable.

After extraction: "I help service business owners implement profit-first so they actually pay themselves every month."

Same expertise. Same guy. Completely different offer.

The shift? He stopped describing his process and started describing the outcome.

That's what the prompts force you to do.

James closed his first client within a month of getting clear.

What outcome does YOUR offer deliver? Not the process. The result.

Michael

---

## Day 7: Behind the Scenes

**Subject:** What my morning actually looks like

---

Hey [NAME],

People think I spend all day creating content.

Here's my actual morning:

- 6:30 — Wake up, no phone for first hour
- 7:30 — Coffee, check Skool for questions
- 8:00 — Write one email (this one)
- 8:30 — Client work or creation
- 12:00 — Done with "deep work"

That's it. The afternoon is calls, admin, whatever.

The point: this business doesn't require 12-hour days. It requires focused mornings and systems that work without you.

The funnel runs. The emails send. The offers convert.

You do the work once. It keeps working.

That's what we're building together.

Michael

P.S. — This is what it looks like when the system runs. If you want help building yours, here's how: the Sprint is 4 weeks, done together. The Blueprint is done for you — I build the whole strategy. Either way, the goal is the same morning I just described.

---

## Day 8: FAQ / Objection

**Subject:** "What if I'm not ready?"

---

Hey [NAME],

Got this question yesterday:

*"I went through the prompts but I'm not sure my offer is 'ready.' What if I put it out there and it flops?"*

Here's the thing:

No offer is ready until it meets the market.

You can tweak your offer document forever. Adjust the wording. Second-guess the price. Wonder if you should add another bonus.

Or you can send it to 5 people and find out what's actually broken.

The first approach feels safe. The second approach gets you data.

Every successful coach I know launched before they felt ready. Then they fixed what didn't work based on real feedback.

Your offer doc isn't a final draft. It's a first hypothesis.

Test it. Learn. Adjust.

That's the game.

Michael

---

## Day 9: The Roadmap

**Subject:** What happens after $27

---

Hey [NAME],

Quick roadmap of where this goes:

**Stage 1: Extraction**
You're here. Define the offer, know who it's for, articulate the transformation.

**Stage 2: Validation**
Test the offer with real people. Warm audience first. Get proof it works before building infrastructure.

**Stage 3: Funnel**
Landing page, email sequence, checkout. The system that runs without you.

**Stage 4: Traffic**
Paid ads, content, whatever drives qualified people to your funnel.

**Stage 5: Scale**
Raise prices. Add tiers. Build a team.

You don't need all five stages right now. You just need to nail Stage 1.

Most coaches skip straight to Stage 4 (traffic) with a broken Stage 1 (offer). They wonder why nobody buys.

Get the foundation right. Everything else gets easier.

Michael

P.S. — If you've finished Stage 1 and you're ready to move, here's the difference between the next steps:

**The Sprint ($297)** — We build Stages 2-4 together over 4 weeks. Weekly calls, direct chat, accountability. Best if you want to do the work yourself with guidance.

**The Blueprint ($397)** — I build your complete strategy, copy, and funnel template for you. Best if you want it done and ready to implement.

No rush. But if you're feeling the momentum — that's the window to use it.

---

## Day 10: Community Invite

**Subject:** Come hang out

---

Hey [NAME],

Last structured email in this sequence. After this, you'll get my daily emails — one story, one offer, one takeaway.

But before that — an invitation.

**Join us in Skool.**

It's where I hang out. It's where other Client Ready members ask questions, share wins, and get unstuck.

Direct link: [SKOOL LINK]

What you'll find:
- The full classroom (Extract → Build → Launch)
- Live call replays
- Direct access to me in the comments
- Other coaches figuring this out together

No pitch. Just community.

See you inside?

Michael

P.S. — Tomorrow you'll start getting my daily emails. Same format: story, offer, CTA. If you love them, great. If you want out, unsubscribe anytime. No hard feelings.

---

## Automation Notes

| Day | Email | Condition | Iron Strike |
|-----|-------|-----------|-------------|
| 0 | Purchase confirmation | Transactional (separate) | — |
| 1 | Welcome + Quick Win | All buyers | — |
| 2 | Origin Story | All buyers | — |
| 3a | Advanced Tips | Buyers who opened product | Consumption branch |
| 3b | Quick Start | Buyers who didn't open product | Consumption branch |
| 4 | Common Mistake | All buyers | — |
| 5 | Quick Tip | All buyers | Soft close P.S. |
| 6 | Transformation Story | All buyers | — |
| 7 | Behind the Scenes | All buyers | Soft close P.S. |
| 8 | FAQ / Objection | All buyers | — |
| 9 | The Roadmap | All buyers | Explicit CTA |
| 10 | Community Invite | All buyers | — |

**After Day 10:** Add to daily broadcast list.

---

## Parallel Sequences (Handle Upsells)

These run alongside welcome — different sending times to avoid stacking:

| Sequence | Days | Sends At | Purpose |
|----------|------|----------|---------|
| Welcome | 1-10 | 8:00 AM | Relationship |
| Bump Recovery | 2, 4, 6 | 2:00 PM | Re-pitch missed bumps |
| OTO Recovery | 3, 5, 7 | 2:00 PM | Re-pitch Sprint/DFY |
| Community Recovery | 8 | 2:00 PM | Downsell to $47/mo |

**Max 2 emails per day.** Morning = value. Afternoon = offer.

---

## Metrics to Track

| Metric | Target |
|--------|--------|
| Open rate | 50%+ (Days 1-3), 40%+ (Days 4-10) |
| Reply rate | 5%+ |
| Skool join rate (Day 10) | 20%+ |
| Unsubscribe rate | <0.3% per email |

---

## Voice Checklist

- [x] Direct, not hype
- [x] Short paragraphs (1-3 sentences)
- [x] Personal stories
- [x] Soft ascension only (P.S. sections on Days 5, 7, 9 — recovery handles hard pitches)
- [x] PS where natural
- [x] Reply invitations
